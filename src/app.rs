...
use pebbles::prelude::*;
use pebbles::widgets::{DesignLanguage, theme_override};

use crate::screens;
use crate::state::{
    DOCS, LANDING, LEARN, NAV, SHOWCASE, design, group_of, navigate, route, to_components,
};
...
fn install_tour() {
    ...
    let Some(ms) = std::env::var("GALLERY_TOUR")
        .ok()
...
pub fn app() -> AnyWidget {
    ...
    let body = route_view(current.clone())
        ...
        .route("text-fields", || {
            component(screens::text_fields::text_fields)
        })
        .route("date-picker", || {
            component(screens::date_picker::date_picker)
        })
        ...
        .route("animated-container", || {
            component(screens::animated_container::animated_containers)
        })
        // Motion & transitions — one screen per widget.
        .route("animated-opacity", || {
            component(screens::motion::animated_opacity_screen)
        })
        .route("animated-scale", || {
            component(screens::motion::animated_scale_screen)
        })
        .route("animated-rotation", || {
            component(screens::motion::animated_rotation_screen)
        })
        .route("animated-slide", || {
            component(screens::motion::animated_slide_screen)
        })
        .route("animated-align", || {
            component(screens::motion::animated_align_screen)
        })
        .route("animated-padding", || {
            component(screens::motion::animated_padding_screen)
        })
        .route("animated-positioned", || {
            component(screens::motion::animated_positioned_screen)
        })
        .route("fade-transition", || {
            component(screens::motion::fade_transition_screen)
        })
        .route("scale-transition", || {
            component(screens::motion::scale_transition_screen)
        })
        .route("rotation-transition", || {
            component(screens::motion::rotation_transition_screen)
        })
        .route("slide-transition", || {
            component(screens::motion::slide_transition_screen)
        })
        .route("size-transition", || {
            component(screens::motion::size_transition_screen)
        })
        .route("positioned-transition", || {
            component(screens::motion::positioned_transition_screen)
        })
        .route("decorated-box-transition", || {
            component(screens::motion::decorated_box_transition_screen)
        })
        .route("animated-switcher", || {
            component(screens::motion::animated_switcher_screen)
        })
        .route("animated-cross-fade", || {
            component(screens::motion::animated_cross_fade_screen)
        })
        .route("dismissible", || {
            component(screens::motion::dismissible_screen)
        })
        .route("animated-list", || {
            component(screens::motion::animated_list_screen)
        })
        .route("animated-grid", || {
            component(screens::motion::animated_grid_screen)
        })
        ...
        .route("drag-target", || {
            component(screens::dnd::drag_target_screen)
        })
        .route("long-press-draggable", || {
            component(screens::dnd::long_press_draggable_screen)
        })
        .route("ignore-pointer", || {
            component(screens::dnd::ignore_pointer_screen)
        })
        .route("absorb-pointer", || {
            component(screens::dnd::absorb_pointer_screen)
        })
        .route("interactive-viewer", || {
            component(screens::dnd::interactive_viewer_screen)
        })
        .route("reorderable-list-view", || {
            component(screens::dnd::reorderable_list_view_screen)
        })
        // Layout — one screen per long-tail layout widget.
        .route("indexed-stack", || {
            component(screens::layout_extra::indexed_stack_screen)
        })
        .route("offstage", || {
            component(screens::layout_extra::offstage_screen)
        })
        .route("visibility", || {
            component(screens::layout_extra::visibility_screen)
        })
        .route("baseline", || {
            component(screens::layout_extra::baseline_screen)
        })
        .route("rotated-box", || {
            component(screens::layout_extra::rotated_box_screen)
        })
        .route("unconstrained-box", || {
            component(screens::layout_extra::unconstrained_box_screen)
        })
        .route("sized-overflow-box", || {
            component(screens::layout_extra::sized_overflow_box_screen)
        })
        .route("fractional-translation", || {
            component(screens::layout_extra::fractional_translation_screen)
        })
        .route("table-layout", || {
            component(screens::layout_extra::table_screen)
        })
        .route("custom-single-child", || {
            component(screens::layout_extra::custom_single_child_layout_screen)
        })
        .route("custom-multi-child", || {
            component(screens::layout_extra::custom_multi_child_layout_screen)
        })
        .route("flow", || component(screens::layout_extra::flow_screen))
        .route("layout-builder", || {
            component(screens::layout_extra::layout_builder_screen)
        })
        // Painting & effects — one screen per widget.
        .route("clip-rect", || {
            component(screens::painting::clip_rect_screen)
        })
        .route("clip-oval", || {
            component(screens::painting::clip_oval_screen)
        })
        .route("clip-path", || {
            component(screens::painting::clip_path_screen)
        })
        .route("color-filtered", || {
            component(screens::painting::color_filtered_screen)
        })
        .route("shader-mask", || {
            component(screens::painting::shader_mask_screen)
        })
        // Input & forms — additions (the base input set has its own screens).
        .route("choice-chip", || {
            component(screens::inputs_extra::choice_chip_screen)
        })
        .route("filter-chip", || {
            component(screens::inputs_extra::filter_chip_screen)
        })
        .route("action-chip", || {
            component(screens::inputs_extra::action_chip_screen)
        })
        .route("stepper", || {
            component(screens::inputs_extra::stepper_screen)
        })
        .route("selectable-text", || {
            component(screens::inputs_extra::selectable_text_screen)
        })
        // Material staples — one screen per widget.
        .route("fab", || component(screens::material::fab_screen))
        .route("grid-tile", || {
            component(screens::material::grid_tile_screen)
        })
        // Async & accessibility long-tail.
        .route("stream-builder", || {
            component(screens::async_a11y::stream_builder_screen)
        })
        .route("semantics-combinators", || {
            component(screens::async_a11y::semantics_combinators_screen)
        })
        // Scaffold slots + mobile-specific.
        .route("media-query", || {
            component(screens::mobile::media_query_screen)
        })
        .route("safe-area", || component(screens::mobile::safe_area_screen))
        .route("orientation-builder", || {
            component(screens::mobile::orientation_builder_screen)
        })
        .route("scaffold-slots", || {
            component(screens::mobile::scaffold_slots_screen)
        })
        // Long-tail.
        .route("placeholder", || {
            component(screens::longtail::placeholder_screen)
        })
        .route("banner", || component(screens::longtail::banner_screen))
        .route("scroll-notification", || {
            component(screens::parity_extra::scroll_notification_screen)
        })
        .route("list-body", || {
            component(screens::parity_extra::list_body_screen)
        })
        .route("list-tiles", || {
            component(screens::parity_extra::list_tiles_screen)
        })
        .route("draggable-sheet", || {
            component(screens::parity_extra::draggable_sheet_screen)
        })
        .route("scaffold-drawer", || {
            component(screens::parity_extra::scaffold_drawer_screen)
        })
        .route("default-text-style", || {
            component(screens::parity_extra::default_text_style_screen)
        })
        .route("mobile-runtime", || {
            component(screens::parity_extra::mobile_runtime_screen)
        })
        .route("nested-scroll", || {
            component(screens::parity_extra::nested_scroll_screen)
        })
        .route("pressable", || {
            component(screens::parity_extra::pressable_screen)
        })
        ...
        .route("collapsible", || {
            component(screens::collapsible::collapsibles)
        })
    ...
    // Design-language preview switcher: a dropdown above the previewed content that
    // re-skins every widget on the screen through Compact / Tailwind / Material. Only
    // the PREVIEW is re-themed (the app's own chrome keeps its theme).
    let dl = design().get();
    let sel_idx = match dl {
        DesignLanguage::Compact => 0,
        DesignLanguage::Tailwind => 1,
        DesignLanguage::Material => 2,
    };
    let design_bar = padding(
        EdgeInsets::symmetric(24.0, 10.0),
        row(children![
            icon(lucide::PALETTE).size(15.0).color(c.muted_foreground),
            gap_w(8.0),
            text("Design")
                .size(12.0)
                .weight(600.0)
                .color(c.muted_foreground),
            gap_w(10.0),
            select(["Compact", "Tailwind", "Material"])
                .value(sel_idx)
                .width(150.0)
                .on_changed(|i, _| {
                    design().set(match i {
    ...
    let previewed = theme_override(theme().design(dl), body);
    let content = column(children![design_bar, expanded(previewed)])
